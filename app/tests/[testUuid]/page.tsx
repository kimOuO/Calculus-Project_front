...
import { getTest, downloadFile } from '@/services/api';
...
export default function TestDetailPage() {
  ...
  const [histogramType, setHistogramType] = useState<string>('');
  ...
  useEffect(() => {
    ...
    return () => {
      // Clean up previous URLs when effect re-runs
      setPaperUrl((prevUrl) => {
        if (prevUrl) URL.revokeObjectURL(prevUrl);
        return null;
      });
      setHistogramUrl((prevUrl) => {
        if (prevUrl) URL.revokeObjectURL(prevUrl);
        return null;
      });
    };
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="bg-white rounded-lg shadow p-6">
            <h2 className="text-xl font-bold mb-4">考卷</h2>
            {paperUrl && !paperError ? (
              <div className="space-y-4">
                {paperType === 'application/pdf' ? (
